    ...
    while gap > 0:
        for i in range(gap, len(a_deep)):
            ...
            while j >= gap and a_deep[j-gap] > a_deep[j]:
                ...
                j -= gap
        gap //= 2
    return print("shell_sort", a_deep)

def qsort_sort(a, l, r):
    if l >= r:
        return
...
for i in range(num):
    ...
    end4 = time.time()
    start5 = time.time()
    qsort_sort(a, 0, length-1)
    end5 = time.time()
    qtimes1.append(end1 - start1)
    qtimes2.append(end1 - start1)
    qtimes3.append(end1 - start1)
    qtimes4.append(end1 - start1)
    qtimes5.append(end1 - start1)


print(nums)
plt.title("Sort Time Comparison")
plt.xlabel("# of date")
plt.xscale("log")
plt.ylabel("Time")
...
plt.xscale("log")
plt.ylabel("Time")
plt.plot(nums, qtimes4, marker = "o", label = "shell")
plt.plot(nums, qtimes5, marker = ".", label = "q")
plt.legend()
plt.show()